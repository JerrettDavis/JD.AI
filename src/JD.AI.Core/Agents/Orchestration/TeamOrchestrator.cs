...
public sealed class TeamOrchestrator
{
        ...
        CancellationToken ct = default)
    {
        ...
        AgentOutput.Current.RenderInfo($"  🏗️ Team ({strategy.Name}): {agents.Count} agent(s) — {goal}");
...
#pragma warning disable CA1031
        ...
        {
            sw.Stop();
            AgentOutput.Current.RenderWarning($"  ⚠ Team failed: {ex.Message}");
            return new TeamResult
            {
...
#pragma warning restore CA1031
        ...
        CancellationToken ct = default)
    {
        ...
        AgentOutput.Current.RenderInfo($"  🔀 Spawning {config.Type} agent '{config.Name}' ({(multiTurn ? "multi-turn" : "single-turn")})...");
