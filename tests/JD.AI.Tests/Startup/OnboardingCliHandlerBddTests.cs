...
public sealed class OnboardingCliHandlerBddTests : TinyBddXunitBase
{
    private static readonly string[] ProjectDefaultsArgs =
        ["--provider", "OpenAI Codex", "--model", "gpt-5.3-codex", "--skip-mcp", "--skip-import"];

    private static readonly string[] GlobalDefaultsArgs =
        ["--provider", "OpenAI Codex", "--model", "gpt-5.2-codex", "--global", "--skip-mcp", "--skip-import"];

    private static readonly string[] UnknownProviderArgs =
        ["--provider", "Unknown Provider", "--model", "gpt-5.3-codex", "--skip-mcp", "--skip-import"];

    private static readonly string[] UnknownModelArgs =
        ["--provider", "OpenAI Codex", "--model", "non-existent-model", "--skip-mcp", "--skip-import"];
    ...
    private static async Task WithTempProjectAsync(Func<string, string, Task> action)
    {
        ...
        var configPath = Path.Combine(userProfile, ".jdai", "config.json");
        var backupPath = configPath + ".onboarding-test-backup";
